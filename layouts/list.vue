...
const { posts, title, currentPage, pagination } = toRefs(props);
...
const filteredBlogPosts = computed(() => {
  ...
  if (pagination) {
    return posts?.value.slice(
      constants.POSTS_PER_PAGE * (currentPage.value - 1),
      currentPage.value * constants.POSTS_PER_PAGE
    );
  }
  return posts;
});
const totalPages = computed(() =>
  Math.ceil(posts.value.length / constants.POSTS_PER_PAGE)
);
</script>
<template>
  ...
  <Pagination
    v-show="totalPages > 1 && !searchValue"
